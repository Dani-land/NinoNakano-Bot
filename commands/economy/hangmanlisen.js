...
export async function before(m, { client }) {
  ...
  if (!primaryBotId || sameJid(primaryBotId, botId)) {
    ...
    if (
      !m.quoted ||
      !games[m.chat] ||
      m.quoted.id !== games[m.chat].messageId ||
      m.sender !== games[m.chat].player
    ) return;
    ...
    try {
      ...
      if (!guess.match(/^[a-z]+$/)) {
        return client.reply(
          m.chat,
          `✦ Envía únicamente letras o la palabra completa, sin números ni símbolos.`,
          m
        );
      }
...
╰━━━━━━━━━━━━⬣`;
          ...
          const usedLetters =
            Array.from(game.guessedLetters).join(', ') || 'Ninguna';
...
✦ Esa palabra no es correcta.`;
      ...
      if (guess.length === 1) {
        ...
        if (game.guessedLetters.has(guess)) {
          ...
          return client.reply(
            m.chat,
            `✦ La letra *${guess}* ya fue utilizada.`,
            m
          );
        }
...
✦ Responde con una letra o la palabra completa.`;
        ...
        await client.reply(
          m.chat,
          `✦ Debes enviar una sola letra o una palabra de ${word.length} letras.`,
          m
        );
      }
