export default {
  ...
  run: async ({client, m, args}) => {
    ...
    if (user.birth)
      return m.reply(
        `✦ Ya tienes una fecha de cumpleaños registrada.\n> Usa › *${prefa}delbirth* para eliminarla.`,
      )
    ...
    if (!birth || birth.includes('El año no puede ser mayor'))
      return m.reply(birth || `✦ Fecha inválida.\n> Usa › *${prefa}setbirth 01/01/2000*`)

    user.birth = birth
    return m.reply(`✿ Tu fecha de nacimiento fue guardada correctamente.\n> 🎂 Cumpleaños › *${user.birth}*`)
  },
};
