...
export default {
  ...
  run: async ({client, m, args}) => {
    ...
    if (!m.quoted) return m.reply(`✎ Cita el mensaje de la waifu para reclamarla.`)
   ...
   // chatConfig.personajesReservados ||= []
    ...
    if (!reservedCharacter) {
      ...
      if (claimedEntry) {
        ...
        if (claimerId === userId) return m.reply(`✰ ¡Has reclamado a *${claimedChar.name}!*.`)
      ...
      return m.reply(`ꕥ No se ha podido reclamar el personaje.`)
    }
    ...
    if (
      ...
      return m.reply(
        `『✐』 El personaje *${reservedCharacter.name}* ya ha expirado hace *${expiredTime}s*.`,
      )
    }
