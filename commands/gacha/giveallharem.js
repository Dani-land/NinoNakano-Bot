...
export default {
  ...
  run: async ({client, m, args}) => {
    ...
    if (!who2 || mentionedJid === senderId)
      return m.reply('✎ Menciona al usuario al que deseas regalar a todos tus personajes.')

    if (!chatData.users?.[senderId]?.characters?.length)
      return m.reply('ꕥ No tienes personajes en tu inventario.')

   const user2 = global.db.data.chats[m.chat].users[mentionedJid]

    if (!user2) {
      return m.reply('《✧》 El usuario *mencionado* no está *registrado* en la base de datos')
    }
    ...
    const message = `✎ Has regalado todos tus personajes a » *${nameReceiver}*.`
