...
export default {
  ...
  run: async ({ client, m, args }) => {
    ...
    if (!text) {
      return client.reply(
        m.chat,
        '『✐』 Ingresa un *término* de búsqueda para buscar tu imagen.',
        m
      )
    }
  ...
  'mia khalifa +18', 'mia khalifa xxx', 'mia khalifa desnuda', 'mia khalifa porno'
    ...
    await m.reply('『✐』 Buscando tu imagen...')
    ...
    try {
      ...
      if (!json.success || !json.data?.result?.length) {
        return m.reply(`ꕥ No se encontraron resultados de *${text}*`)
      }
      ...
      console.error(e)
      await m.reply('ꕥ Error al obtener la imagen.')
    }
  }
}