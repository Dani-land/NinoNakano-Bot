...
import { resolveLidToRealJid, fetchIconBuffer } from "./utils.js"
...
export async function fixLid2(client, m) {
  const decodedJid = client.decodeJid(m.msg?.contextInfo?.participant || m.chat || '')
...
export async function smsg(client, m, store) {
  ...
  if (m.key) {
    m.id = m.key.id
    m.chat = client.decodeJid(m.key.remoteJid)
    ...
    m.isBot =
      ...
      false
    // En conversaciones privadas remoteJid también puede llegar como LID.
    // Resolverlo aquí permite que owner, base de datos y comandos usen el PN.
    if (m.chat?.endsWith('@lid')) {
      m.chat = await resolveLidToRealJid(m.chat, client, m.chat)
    }
  ...
  if (m.message) {
    ...
    if (m.quoted) {
      ...
      m.getQuotedObj = async () => {
        if (!m.quoted.id || !store?.loadMessage) return false
  ...
  m.copy = () =>
    smsg(client, proto.WebMessageInfo.fromObject(proto.WebMessageInfo.toObject(m)), store)
  ...
  .reduce((acc, [key, value]) => {
    ...
    const canalSeleccionado = canales.length
      ? canales[Math.floor(Math.random() * canales.length)]
      : { id: '120363420575743790@newsletter', nombre: botname || 'Nino Nakano Bot' }

    // Descargar ícono como Buffer para que externalAdReply lo muestre correctamente
    const _iconBuf = await fetchIconBuffer(icon)
    ...
    const dynamicButtons = buttons.map((btn) => ({
      ...
      contextInfo: {
        ...
        externalAdReply: {
          title: botname,
          body: botname2 + ', Built With ♡ By Danuelrxz',
          mediaType: 1,
          renderLargerThumbnail: false,
          previewType: `PHOTO`,
          ...(_iconBuf ? { thumbnail: _iconBuf } : { thumbnailUrl: icon }),
          sourceUrl: global.redes || global.bot?.web || '',
        },
      },
    }))
  ...
  const pfp = botSetting.icon || ''
    const canalI = botSetting.id || '120363420575743790@newsletter'
    const canalNam = botSetting.nameid || 'ミ★ 𝙉𝙞𝙣𝙤 𝙐𝙥𝙙𝙖𝙩𝙚𝙨 ★彡'
  ...
  client.sendContextInfo = async (jid, text = '', options, quoted) => {
    ...
    const _pfpBuf = await fetchIconBuffer(pfp)
      ...
      jid,
      {
        extendedTextMessage: {
          ...
          contextInfo: {
            ...
            externalAdReply: {
              title: namebugsito,
              body: `${namebugg2}, Built With ꕤ By NinoNakano`,
              mediaType: 1,
              renderLargerThumbnail: false,
              previewType: `PHOTO`,
              ...(_pfpBuf ? { thumbnail: _pfpBuf } : { thumbnailUrl: pfp }),
               sourceUrl: global.bot?.api || '',
            },
          },
        },
      },
  ...
  client.sendContextInfoIndex = async (
    ...
    const namebot2 = config.body || `${settings.namebot2}, Built With ꕤ By NinoNakano`
    const canalId = settings.id ||'120363420575743790@newsletter'
    const canalName = settings.nameid || 'ミ★ 𝙉𝙞𝙣𝙤 𝙐𝙥𝙙𝙖𝙩𝙚𝙨 ★彡'
    const sourceUrl =
    ...
    // Descargar ícono como Buffer para que el thumbnail aparezca correctamente
    const _bannerBuf = await fetchIconBuffer(banner)
    const content = {
      extendedTextMessage: {
        text,
        contextInfo: {
          ...
          externalAdReply: {
            ...
            renderLargerThumbnail: false,
            previewType: 'PHOTO',
            ...(_bannerBuf ? { thumbnail: _bannerBuf } : { thumbnailUrl: banner }),
            sourceUrl
          }
        }
      }
