...
export default async (client, m) => {
    ...
    const botJidLid   = client.user?.lid ? stripDevice(client.decodeJid(client.user.lid)) : null

    // Helper: ¿este participante es el bot? (compara por teléfono Y por LID)
    const isParticipantBot = (p) =>
        stripDevice(p.id) === botJidPhone ||
